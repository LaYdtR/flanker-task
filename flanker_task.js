...
let expName = 'flanker_task';  // from the Builder filename that created this script
let expInfo = {
...
async function experimentInit() {
  ...
  instr_text = new visual.TextStim({
    win: psychoJS.window,
    name: 'instr_text',
    text: 'Flanker Task\n\nYou will see rows of arrows\n\nPress the LEFT ARROW key if the MIDDLE arrow points left\nPress the RIGHT ARROW key if the MIDDLE arrow points right\n\nTry to respond as quickly and accurately as possible\n\nThere will be 4 practice trials first\nThen 40 real trials\n\nPress SPACEBAR to begin practice',
    font: 'Arial',
    units: undefined, 
  ...
  return async function () {
    ...
    if (key_resp_prac.status === PsychoJS.Status.STARTED && t >= frameRemains) {
      ...
      if (key_resp_prac.status === PsychoJS.Status.STARTED) {
        let theseKeys = key_resp_prac.getKeys({keyList: ['left','right'], waitRelease: false});
    ...
    return async function () {
      ...
      if (key_resp2.status === PsychoJS.Status.STARTED && t >= frameRemains) {
        ...
        if (key_resp2.status === PsychoJS.Status.STARTED) {
          let theseKeys = key_resp2.getKeys({keyList: ['left','right'], waitRelease: false});
